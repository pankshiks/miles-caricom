...
/**
 * Plugin implementation of the 'color_pickr_hexagon' formatter.
 *
 * @FieldFormatter(
 *   id = "color_pickr_hexagon",
 *   label = @Translation("Color Pickr Hexagon"),
 *   field_types = {
 *     "color_pickr_code"
 *   }
 * )
 */
class ColorPickrHexagonFormatter extends FormatterBase {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      // Render output using color_pickr_hexagon theme.
      if (isset($item->color_pickr) && $item->color_pickr != 'none') {
        $source = [
          '#theme' => 'color_pickr_hexagon',
